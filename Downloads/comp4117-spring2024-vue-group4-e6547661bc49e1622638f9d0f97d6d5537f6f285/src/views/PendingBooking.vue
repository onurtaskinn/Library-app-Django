...
<td data-label="Requestion">{{ booking.request ? booking.request : '' }}</td>
      ...
      <pagination :current-page="currentPage" :total-pages="totalPages" @update:currentPage="fetchPendingBookings" />
...
async function fetchPendingBookings(page = 1) {
  const response = await bookingsService.getPendingBooking({ page });
  response.data.forEach(booking => {
      booking.createdAt = formatMongoDBDate(booking.createdAt);
  
      });
  currentPaginationFunction.value = fetchPendingBookings;  // Update current function
...
onMounted(() => {
  fetchPendingBookings(currentPage.value);
});
...
async function submitApproval(recordId, approvalStatus) {
  ...
  try {
    ...
    if(confirm('Are you sure ' + (approvalStatus === 'approved' ? 'approval' : 'disapproval') + " record " + recordId , '?')){
      await bookingsService.updateBooking(recordId, recordToUpdate);
