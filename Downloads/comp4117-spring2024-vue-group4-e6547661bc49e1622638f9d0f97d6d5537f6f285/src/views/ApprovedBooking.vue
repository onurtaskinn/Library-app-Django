      ...
      <div class="d-flex justify-content-between align-items-center mb-4">
          <div class="text-center flex-grow-1">
            <div class="title" style="float: left;">
              ...
              <div>Number of Approved Booking {{ totalRecord }}</div>
            </div>
          </div>
          <div class="toolBar">
          </div>
...
async function fetchApprovedBookings(page = 1) {
  const response = await bookingsService.getApprovedBooking({ page });
  response.data.forEach(booking => {
      booking.createdAt = formatMongoDBDate(booking.createdAt);
  
      });
  currentPaginationFunction.value = fetchApprovedBookings;  // Update current function
  
  bookings.value = response.data;
  totalPages.value = response.totalPages;
  currentPage.value = response.page;
  totalRecord.value = response.total;
  subTitle.value = "Approved Bookings";
}
...
onMounted(() => {
  fetchApprovedBookings(currentPage.value);
});
