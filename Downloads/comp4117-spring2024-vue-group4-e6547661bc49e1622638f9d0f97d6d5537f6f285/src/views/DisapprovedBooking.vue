...
<table>
  ...
  <tbody>
    <tr v-for="booking in displayBookings" :key="booking._id" :class="booking.approvement">
      ...
      <td  data-label="Booker Id" style="  word-wrap: break-word;">{{ booking._id }}</td>
...
<td data-label="Requestion">{{ booking.request ? booking.request : '' }}</td>
      ...
      <td data-label="Approval">
        ...
        <div v-else>
          <a class="btn-a" @click="submitApproval(booking._id, 'approved')" style="margin: 8px;">Approval</a>
        </div>

        <div v-if="booking.approvement == 'disapproved'">
        </div>
        <div v-else>
          <a class="btn-dis" @click="submitApproval(booking._id, 'disapproved')" style="margin: 8px;">Disapproval</a>
        </div>
...
async function fetchDisapprovedBooking(page = 1) {
  const response = await bookingsService.getDisapprovedBooking({ page });
  response.data.forEach(booking => {
      booking.createdAt = formatMongoDBDate(booking.createdAt);
  
      });
  currentPaginationFunction.value = fetchDisapprovedBooking;  // Update current function
...
onMounted(() => {
  fetchDisapprovedBooking(currentPage.value);
});
